...
(() => {
  const CONFIG = {
    ...
    // Replace with the verified Web3Jobs treasury wallet before enabling live payments.
    paymentWallet: ""
  };
  ...
  async function pay(planCode) {
    ...
    const plan = state.plans.find(p => p.plan_code === planCode);
    ...
    const token = new ethers.Contract(CONFIG.usdtAddress,
      ["function decimals() view returns (uint8)", "function transfer(address,uint256) returns (bool)"], signer);
    ...
    return { userId: user.id, planId: plan.id, planCode: plan.plan_code, planName: plan.plan_name,
      amount: Number(plan.price), currency: plan.currency, walletAddress,
      transactionHash: receipt.hash, blockchainNetwork: "BSC", durationDays: plan.duration_days };
  }

  // Intentionally no client-side INSERT/UPDATE to payments/subscriptions.
  // A trusted verifier must validate the transaction before activation.
