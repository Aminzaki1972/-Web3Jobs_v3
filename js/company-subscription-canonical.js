...
(() => {
  const PLANS = {
    ...
    professional: { name: "Professional", price: 25, duration: 30 },
    enterprise: { name: "Enterprise", price: 50, duration: 30 }
  };
  const BSC = "0x38";
  ...
  const planFrom = el => {
    ...
    for (let i = 0; i < 10 && n; i++, n = n.parentElement) {
      const raw = `${n.dataset?.plan || ""} ${n.dataset?.payPlan || ""} ${n.id || ""} ${n.textContent || ""}`.toLowerCase();
      const code = Object.keys(PLANS).find(k => raw.includes(k));
      if (code) return code;
    }
    return null;
  };
  const ensureBSC = async () => {
    ...
    if (chain !== BSC) await window.ethereum.request({ method: "wallet_switchEthereumChain", params: [{ chainId: BSC }] });
  };
  const connect = async () => {
    await ensureBSC();
  ...
  const css = () => {
    ...
    const s = document.createElement("style"); s.id = "wj-canonical-sub-style";
    s.textContent = `#wj-canonical-sub{position:fixed;inset:0;z-index:2147483647;display:flex;align-items:center;justify-content:center;padding:20px;background:rgba(0,0,0,.78);font-family:Arial,sans-serif}#wj-canonical-sub .box{width:min(560px,100%);padding:24px;border:1px solid #294663;border-radius:18px;background:#081423;color:#fff}#wj-canonical-sub h2{margin:0 0 8px}#wj-canonical-sub p{color:#9cafc5;font-size:12px;line-height:1.5}.wjc-row{display:flex;justify-content:space-between;gap:12px;padding:9px 0;border-bottom:1px solid rgba(255,255,255,.08);font-size:12px}.wjc-green{color:#6ee7b7}.wjc-actions{display:flex;gap:10px;margin-top:18px}.wjc-actions button{flex:1;min-height:44px;border:0;border-radius:10px;font-weight:800;cursor:pointer}.wjc-cancel{background:#17263a;color:#fff}.wjc-action{background:#1c765a;color:#fff}.wjc-status{margin-top:14px;padding:12px;border-radius:10px;background:#0d1d31;color:#b9c9dc;font-size:12px}.wjc-error{display:none;margin-top:12px;padding:10px;border-radius:10px;background:#3a1520;color:#ffb4c0;font-size:12px}`;
    document.head.appendChild(s);
  };
  const open = code => {
    const plan = PLANS[code]; if (!plan) return;
    css();
    const old = document.getElementById("wj-canonical-sub"); if (old) old.remove();
    const m = document.createElement("div"); m.id = "wj-canonical-sub";
    ...
    btn.onclick = async () => {
      ...
      try {
        if (!verified) {
          ...
          btn.disabled = false; btn.textContent = `Pay ${plan.price} USDT`;
        } else {
        ...
        error.textContent = e?.code === 4001 ? "Wallet signature/transaction was cancelled." : (e?.message || String(e));
        error.style.display = "block"; btn.disabled = false;
  ...
  const init = () => {
    ...
    document.addEventListener("click", e => {
      ...
      if (!b) return;
      const code = planFrom(b); if (!code) return;
      e.preventDefault(); e.stopImmediatePropagation(); open(code);
    }, true);
  };
